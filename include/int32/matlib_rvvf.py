...
def get_vector(seq, param, type: Type):
    if not seq.vectors.get(param):
        if type == Type.SCALAR:
            ...
            no = Unroller.idx
            seq.vectors[param] = no
            init = f'vint32_t vec_{no}; double *ptr_{no} = {param};'
            seq.inits.append(init)
            if type == Type.INPUT:
                load = lambda l, vl : f'vec_{no} = __riscv_vle64_v_i32(ptr_{no} + {l}, {vl});'
                seq.loads.append(load)
        Unroller.idx += 1
    else:
        no = seq.vectors.get(param)
    if type == Type.OUTPUT:
        store = lambda l, vl : f'__riscv_vse32_v_i32(ptr_{no} + {l}, vec_{no}, {vl});'
        seq.stores[no] = store
    return no
...
def get_ops_lambda(ops, seq, params):
    ...
    lambdas = {
        'matneg':   lambda vl: f'vec_{nos[1]} = __riscv_vfneg_v_i32(vec_{nos[0]}, {vl});',
        'cwiseabs': lambda vl: f'vec_{nos[1]} = __riscv_vfabs_v_i32(vec_{nos[0]}, {vl});',
        'cwisemin': lambda vl: f'vec_{nos[2]} = __riscv_vfmin_vv_i32(vec_{nos[0]}, vec_{nos[1]}, {vl});',
        'cwisemax': lambda vl: f'vec_{nos[2]} = __riscv_vfmax_vv_i32(vec_{nos[0]}, vec_{nos[1]}, {vl});',
        'cwisemul': lambda vl: f'vec_{nos[2]} = __riscv_vfmul_vv_i32(vec_{nos[0]}, vec_{nos[1]}, {vl});',
        'matmulf':  lambda vl: f'vec_{nos[1]} = __riscv_vfmul_vf_i32(vec_{nos[0]}, {nos[2]}, {vl});',
        'matsub':   lambda vl: f'vec_{nos[2]} = __riscv_vfsub_vv_i32(vec_{nos[0]}, vec_{nos[1]}, {vl});',
        'matadd':   lambda vl: f'vec_{nos[2]} = __riscv_vfadd_vv_i32(vec_{nos[0]}, vec_{nos[1]}, {vl});',
        'matset':   lambda vl: f'vec_{nos[0]} = __riscv_vfmv_v_f_i32({nos[1]}, {vl});'
    }
